...
const CoachProfileScreen = lazy(() => import('./screens/CoachProfileScreen'));
...
const ChampionScreen = lazy(() => import('./screens/ChampionScreen'));
...
export default function PlayApp({ onBackHome, initialIntent = null }: PlayAppProps) {
  ...
  // Staff, Infrastructure, Training & Youth
  const [hiredStaff, setHiredStaff] = useState<StaffMember[]>([]);
  const [staffMarket] = useState<StaffMember[]>([
    { id: 's1', name: 'Carlos Preparador', type: 'COACH', level: 'BRONZE', salary: 15000, description: '+5% OVR em treinos ofensivos', bonus: 5 },
    { id: 's2', name: 'João Fisioterapeuta', type: 'PHYSIO', level: 'BRONZE', salary: 12000, description: '-20% tempo de lesão', bonus: 20 },
    { id: 's3', name: 'Marcos Scout', type: 'SCOUT', level: 'BRONZE', salary: 10000, description: '+3 revelações por temporada', bonus: 3 },
    { id: 's4', name: 'Roberto Tático', type: 'COACH', level: 'SILVER', salary: 28000, description: '+10% OVR em treinos táticos', bonus: 10 },
    { id: 's5', name: 'Ana Médica', type: 'PHYSIO', level: 'SILVER', salary: 25000, description: '-40% tempo de lesão + prevenção', bonus: 40 },
    { id: 's6', name: 'Pedro Olheiro', type: 'SCOUT', level: 'SILVER', salary: 22000, description: '+6 revelações + melhor qualidade', bonus: 6 },
    { id: 's7', name: 'Fábio Maestro', type: 'COACH', level: 'GOLD', salary: 55000, description: '+18% OVR global + moral +5', bonus: 18 },
    { id: 's8', name: 'Dra. Silva', type: 'PHYSIO', level: 'GOLD', salary: 50000, description: 'Eliminação de lesões graves', bonus: 60 },
    { id: 's9', name: 'Mestre Olheiro', type: 'SCOUT', level: 'GOLD', salary: 45000, description: '+10 revelações + jóias internacionais', bonus: 10 },
  ]);
  const [infrastructure, setInfrastructure] = useState<Infrastructure>({ ct: 1, dm: 1, scout: 1 });
  const [squadFocus, setSquadFocus] = useState<TrainingFocus>('BALANCEADO');
  const [trainingIntensity, setTrainingIntensity] = useState<TrainingIntensity>('MEDIA');
  const [youthRoster, setYouthRoster] = useState<Player[]>([]);
  ...
  const applySave = useCallback((save: SaveGame) => {
    ...
    setHiredStaff(save.hiredStaff ?? []);
    setInfrastructure(save.infrastructure ?? { ct: 1, dm: 1, scout: 1 });
  ...
  const saveToSlot = useCallback(
    (slot: SaveSlotId) => {
      ...
      try {
        ...
        writeSlot(slot, save);
        setLastLocalSaveAt(save.savedAt);
  ...
  const loadFromSlot = useCallback(
    (slot: SaveSlotId) => {
      ...
      const isMidGame = !!userTeamId && currentScreen !== 'SPLASH' && currentScreen !== 'COACH_SETUP' && currentScreen !== 'TEAM_SELECT';
      if (isMidGame && !window.confirm(`Carregar o Slot ${slot}? O progresso atual não salvo no slot ativo pode ser perdido.`)) {
        return;
      }

      setActiveSlot(slot);
      setLastLocalSaveAt(s.savedAt);
      applySave(s);
    },
    [applySave, currentScreen, userTeamId]
  );

  const clearSlot = useCallback((slot: SaveSlotId) => {
    const target = readSlot(slot);
    if (!target) {
      toast.error(`Slot ${slot} está vazio.`);
      return;
    }
    ...
    const message = isActiveSlot
      ? `Apagar o Slot ${slot}? Ele é o slot ativo de autosave.`
      : `Apagar o Slot ${slot}? Esta ação não pode ser desfeita.`;
  ...
  const importSaveFromFile = useCallback(
    async (file: File) => {
      try {
        ...
        if (!isValidSaveGame(obj)) {
          toast.error('Arquivo inválido ou incompleto.');
          return;
        }

        if (!window.confirm(`Importar este arquivo para o Slot ${activeSlot}? O conteúdo atual desse slot será sobrescrito.`)) {
          return;
        }
        ...
        try {
          ...
          setLastLocalSaveAt(obj.savedAt);
        } catch { }
        toast.success(`Save importado para o Slot ${activeSlot}.`, { icon: '📥' });
      } catch {
  ...
  const handleMatchFinished = (userGoals: number, opponentGoals: number, events: MatchEvent[]) => {
    ...
    const teamsInDivision = teams.filter(t => t.division === userTeam.division);
    const totalRounds = (teamsInDivision.length - 1) * 2;
      ...
      return {
        ...
        roster: t.roster.map(p => {
          ...
          newHistory.unshift({
            ...
            season,
            type: 'CONTRACT',
            description: `Renovou contrato por mais ${(teams.filter(t => t.division === userTeam.division).length - 1) * 2} rodadas.`,
            icon: '✍️'
          });

          return {
            ...p,
            contractRounds: p.contractRounds + ((teams.filter(t => t.division === userTeam.division).length - 1) * 2),
            history: newHistory
          };
        })
      };
  ...
  const handleStartCareer = useCallback(() => {
    ...
    setCoach(null);
    setHiredStaff([]);
    setInfrastructure({ ct: 1, dm: 1, scout: 1 });
  ...
  const handleContinueCareer = useCallback(() => {
    const latest = getLatestLocalSave();
    if (latest) {
      setActiveSlot(latest.slot);
      setLastLocalSaveAt(latest.save.savedAt);
      applySave(latest.save);
      toast.success(`Retomando o Slot ${latest.slot}.`, { icon: '⏯️' });
    } else {
      toast.error('Nenhum save encontrado.');
    }
  }, [applySave]);
  ...
  useEffect(() => {
    ...
    return runWhenIdle(() => {
      void import('./screens/MarketScreen');
      void import('./screens/FinanceScreen');
      void import('./screens/LeagueScreen');
    ...
    <div className="min-h-screen bg-background text-on-surface font-sans selection:bg-primary/30 relative overflow-hidden h-full">
      <Toaster
        ...
        toastOptions={{
          ...
          style: {
            ...
            color: '#f2fff8',
            border: '1px solid rgba(78, 205, 164, 0.22)',
          ...
          success: {
            ...
            style: {
              ...
              color: '#f3fff8',
              border: '1px solid rgba(31, 177, 133, 0.3)',
            },
          },
          error: {
            iconTheme: {
            ...
            style: {
              background: 'rgba(36, 13, 13, 0.98)',
              color: '#fff5f5',
              border: '1px solid rgba(224, 90, 90, 0.34)',
            },
          },
        }}
      />
      ...
      <AnimatePresence mode="wait">
        <Suspense fallback={<ScreenFallback />}>
          ...
          {currentScreen === 'DASHBOARD' && userTeam && (
            <PageWrapper id="dashboard">
              <DashboardScreen
                ...
                news={news}
                onCompleteOnboarding={() => setHasSeenOnboarding(true)}
                onOpenSquad={() => { setLastScreen('DASHBOARD'); setCurrentScreen('SQUAD'); }}
          ...
          {currentScreen === 'PRE_MATCH' && userTeam && nextOpponent && (
            <PageWrapper id="pre_match">
              <PreMatchScreen
                userTeam={userTeam}
          ...
          {currentScreen === 'NEWS' && (
            <PageWrapper id="news">
              <NewsScreen
                news={news}
                onRead={(id) => setNews(prev => prev.map(n => n.id === id ? { ...n, isRead: true } : n))}
                onChoice={handleNewsChoice}
                onBack={() => setCurrentScreen('DASHBOARD')}
              />
            </PageWrapper>
          )}
          ...
          {currentScreen === 'LEAGUE' && (
            <PageWrapper id="league">
              <LeagueScreen teams={teams} userTeamId={userTeamId} onBack={() => setCurrentScreen('DASHBOARD')} />
            </PageWrapper>
          )}
          ...
          {currentScreen === 'PROFILE' && coach && userTeam && (
          ...
          {currentScreen === 'MARKET' && userTeam && (
            <PageWrapper id="market">
              <MarketScreen userTeam={userTeam} allTeams={teams} funds={funds} isWindowOpen={isWindowOpen} offers={[]} logs={[]} onBack={() => setCurrentScreen('DASHBOARD')} onBuy={handleBuyPlayer} onLoanPlayer={() => { }} onSell={() => { }} onAcceptOffer={() => { }} onDeclineOffer={() => { }} />
            </PageWrapper>
          )}
          ...
          {currentScreen === 'GAME_OVER' && (
            <PageWrapper id="game_over">
              <GameOverScreen reason={gameOverReason} onRestart={handleRestart} />
            </PageWrapper>
          )}

          {currentScreen === 'CHAMPION' && userTeam && (
            <PageWrapper id="champion">
              <ChampionScreen champion={getChampion()} userTeam={userTeam} onNewSeason={handleNextSeason} onQuit={handleRestart} teams={teams} pastSeasons={pastSeasons} />
            </PageWrapper>
          )}

          {(currentScreen as string) === 'TRAINING' && userTeam && (
            <PageWrapper id="training">
              <TrainingScreen
                team={userTeam}
                funds={funds}
                infrastructure={infrastructure}
                hiredStaff={hiredStaff}
                squadFocus={squadFocus}
                intensity={trainingIntensity}
                onSetSquadFocus={(f) => setSquadFocus(f)}
                onSetIntensity={(i) => setTrainingIntensity(i)}
                onBack={() => setCurrentScreen('DASHBOARD')}
              />
            </PageWrapper>
          )}

          {(currentScreen as string) === 'STAFF' && (
            <PageWrapper id="staff">
              <StaffScreen
                hiredStaff={hiredStaff}
                staffMarket={staffMarket}
                funds={funds}
                onHire={(s) => {
                  setHiredStaff(prev => [...prev, s]);
                  setFunds(f => f - s.salary * 3);
                  toast.success(`${s.name} contratado!`, { icon: '🤝' });
                }}
                onFire={(id) => {
                  setHiredStaff(prev => prev.filter(s => s.id !== id));
                  toast.success('Staff dispensado.', { icon: '👋' });
                }}
                onBack={() => setCurrentScreen('DASHBOARD')}
              />
            </PageWrapper>
          ...
          {(currentScreen as string) === 'INFRASTRUCTURE' && userTeam && (
            <PageWrapper id="infra">
              <InfrastructureScreen
                infrastructure={infrastructure}
                funds={funds}
                onUpgrade={(type) => {
                  const cost = (infrastructure[type] + 1) * 150000;
                  if (funds >= cost) {
                    setInfrastructure(prev => ({ ...prev, [type]: prev[type] + 1 }));
                    setFunds(f => f - cost);
                    toast.success('Infraestrutura melhorada!', { icon: '🏗️' });
                  } else {
                    toast.error('Saldo insuficiente.', { icon: '💸' });
                  }
                }}
                onBack={() => setCurrentScreen('DASHBOARD')}
              />
          ...
          {(currentScreen as string) === 'YOUTH' && userTeam && (
            <PageWrapper id="youth">
              <YouthAcademyScreen
                userTeam={userTeam}
                funds={funds}
                infrastructure={infrastructure}
                ...
                onPromote={(player) => {
                  setTeams(prev => prev.map(t =>
                    t.id === userTeamId ? { ...t, roster: [...t.roster, player] } : t
                  ));
                  setYouthRoster(prev => prev.filter(p => p.id !== player.id));
                  toast.success(`${player.name} promovido ao profissional!`, { icon: '⭐' });
                }}
                onBack={() => setCurrentScreen('DASHBOARD')}
              />
            </PageWrapper>
